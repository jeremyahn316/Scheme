...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        evalexpr = scheme_eval(first,env)
        if isinstance(evalexpr,MacroProcedure):
            return scheme_eval(evalexpr.apply_macro(rest, env), env)
        return scheme_apply(evalexpr,rest.map(lambda x:scheme_eval(x,env)),env)
        # END PROBLEM 5
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    rest_of = expressions.second
    return LambdaProcedure(formals,rest_of,env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_form(binded, 2, 2)

        values = Pair(scheme_eval(binded.second.first, env), values)
        pair = Pair(binded.first, pair)

        bindings = bindings.second
    check_formals(pair)
    return env.make_child_frame(pair, values)
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, expressions, env):
        return env.make_child_frame(self.formals, expressions)
